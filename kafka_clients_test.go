...
import (
	...
	ckafkago "github.com/confluentinc/confluent-kafka-go/v2/kafka"
...
func BenchmarkFranzGoAsync(b *testing.B) {
	b.ReportAllocs()
	client, err := kgo.NewClient(
		kgo.SeedBrokers("localhost:29092"),
		kgo.DefaultProduceTopic("test"),
	)
	if err != nil {
		panic(err)
	}
	...
	record := &kgo.Record{Value: m}
	c := context.Background()

	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		client.Produce(c, record, nil)
	}
}

func BenchmarkFranzGoAsyncInParalell(b *testing.B) {
	b.ReportAllocs()
	client, err := kgo.NewClient(
		kgo.SeedBrokers("localhost:29092"),
		kgo.DefaultProduceTopic("test"),
